defmodule FishingSpot.Queries do
  ...
  def all_fishermen do
    ...
    Repo.all(from(fisherman in Fisherman, select: fisherman)) |> IO.inspect()

    Repo.all(from(fisherman in Fisherman, select: [fisherman.name, fisherman.date_of_birth]))
    |> IO.inspect()
  ...
  def biggest_fish do
    Repo.all(from(fish in FishLanded, select: max(fish.length))) |> IO.inspect()

    Repo.all(from(fish in FishLanded, select: {max(fish.length)})) |> IO.inspect()

    Repo.all(from(fish in FishLanded, select: [max(fish.length)])) |> IO.inspect()

    Repo.all(from(fish in FishLanded, select: %{big_fish: max(fish.length)}))
  end

  def fishy_fish do
    Repo.all(from(fish in FishLanded, where: fish.length > 24))
  end

  def fisherman_by_keyword do
    {_, date} = Ecto.Date.cast("1976-01-05")

    Repo.all(from(fisherman in Fisherman, where: [name: "Lew", date_of_birth: ^date]))

    where(Fisherman, name: "Lew", date_of_birth: ^date) |> Repo.all()

    join(Fisherman, :inner, [], fish_landed in FishLanded)
    |> where(
      [fisherman, fish_landed],
      name: "Lew",
      date_of_birth: ^date,
  ...
  def biggest_fish_per_fisherman_two_queries do
    [big_fish] = Repo.all(from(fish in FishLanded, select: max(fish.length)))
  ...
  def distinct_fish_weight_with_expression do
    query = from(fish in FishLanded, distinct: fish.weight)
  ...
  def all_fish_limit_10 do
    Repo.all(from(fish in FishLanded, limit: 10))
  end
